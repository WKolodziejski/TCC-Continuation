  ...
  while (MIN_TRIALS > trial_count) {
    ...
    if (current_motion.num_inliers >= worst_kept_motion->num_inliers &&
      ...
      if (is_better_motion(&current_motion, worst_kept_motion)) {
        ...
        bestiteration = trial_count;
        bestloss = current_motion.variance;
      }
    }
    trial_count++;
  }
...
RansacFunc av1_get_ransac_type(enum TransformationType type) {
  switch (type) {
...
RansacFuncDouble av1_get_ransac_double_prec_type(enum TransformationType type) {
  switch (type) {
